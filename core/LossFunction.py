...
class PointLossFunction(Module):
    def __init__(
        self, resolution, renderer, device, settings, debug, num_views, logger
    ):
        super().__init__()
        ...
        self.pos = (
            torch.cat([pos[1][..., None], pos[0][..., None]], dim=2)
            .to(self.device)[None, ...]
    ...
    def match_Sinkhorn(self, haspos, render_point_5d, gt_rgb, view):
        ...
        target_point_5d[..., :3] = (
            target_point_5d[..., :3] * self.rgb_match_weight(view)
        )
        pointloss = (
    ...
    def forward(self, gt, iteration=-1, scene=None, view=0):
        ...
        render_pos[not haspos] = self.pos[view : view + 1][not haspos].clone()
        render_point_5d = torch.cat([render_rgb, render_pos], dim=-1)
